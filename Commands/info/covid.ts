...
export = class extends Command {
  ...
  constructor(client: Client) {
    super(
      ...
      {
        description:
          "Find out the latest statistics for the coronavirus pandemic.",
  ...
  async run(msg: Message) {
    const arg = msg.args[0]?.toLowerCase() || "world";
    let data;
    let embed;
    switch (arg) {
      case "world":
      ...
      default:
        ...
        if (!data)
          return await msg.send(
            `${this.client.config.emojis.cross} The country \`${msg.args[0]}\` was not found`
          );
        embed = {
