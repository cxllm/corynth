  ...
  async run(msg: Message) {
    ...
    switch (arg) {
      case "on":
        ...
        try {
          channels = await msg.channel.awaitMessages(
        ...
        let chan =
          channels.first().mentions.channels.first() ||
          (await msg.guild.channels.fetch(channels.first().content, false));
      ...
      case "off":
        ...
        await this.client.db.guilds.set(msg.guild.id, msg.db);
        await msg.send(
          `${this.client.config.emojis.tick} Leave message turned off`
        );
        break;
      case "view":
        ...
        embed = {
          title: "Leave Message",
          description: leave
            ? leave.message && leave.channel
              ? `The leave message is set to \`${leave.message}\` & the channel is <#${leave.channel}> `
              : `The leave message is off`
            : `The leave message is off`,
          color: this.client.config.colours.main,
        };
