		...
		if (guilddb.autorole) {
			if (!member.guild.me.permissions.has("MANAGE_ROLES")) {
				...
				if (member.user.bot && guilddb.autorole.bot) {
					...
					await member.roles.add(guilddb.autorole.bot);
				}
				if (guilddb.autorole.user) {
					if (!member.guild.roles.cache.has(guilddb.autorole.user)) {
						guilddb.autorole.user = null;
						return await this.client.db.guilds.set(member.guild.id, guilddb);
					}
					await member.roles.add(guilddb.autorole.user);
				}
			}
		}
		if (guilddb.join) {
			if (guilddb.join && guilddb.join.channel && guilddb.join.message) {
				let channel;
				try {
					channel = await member.guild.channels.fetch(guilddb.join.channel, {
						cache: false
					});
				} catch {
					guilddb.join = {};
