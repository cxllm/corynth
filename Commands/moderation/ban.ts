  ...
  async run(msg: Message) {
    ...
    if (user.id === msg.author.id)
      return await msg.send(
        `${this.client.config.emojis.cross} You can't ban yourself!`
      );
    ...
    if (user instanceof User) {
      ...
      if (
        ...
        return await msg.send(
          `${this.client.config.emojis.cross} You can't ban this person because their position is equal to or greater than yours!`
        );
      if (!user.bannable)
        return await msg.send(
          `${this.client.config.emojis.cross} I can't ban this person because their position is equal to or greater than mine!`
        );
    }
    try {
      ...
      try {
        await user.send(
          `You have been banned from ${msg.guild.name} for the reason: \`${reason}\``
        );
      } catch {}
