...
export default class Util {
				...
				cheerio
					...
					.map((el, i) => {
						...
						return {
							...
							artist: element(".chartlist-artist").text().split("\n").join("").trim(),
							scrobbles: scrobbles
								? parseInt(
							...
							time: scrobbles
								? null
								: element(".chartlist-timestamp").text().split("\n").join("").trim(),
							artist_url:
								"https://www.last.fm" +
								element(".chartlist-artist a").attr().href.replace(")", "\\)"),
							song_url:
								"https://www.last.fm" +
								element(".chartlist-name a").attr().href.replace(")", "\\)")
						};
					})
			);
		};
	...
	async getMember(ctx: Message, query?: string, cache?: boolean) {
		...
		ctx.guild.members.fetch({ user: out.id, cache: cache ?? true });
		return out;
	}
	...
	async getPunishmentUser(
		...
		try {
			if ((!user || user.id === msg.author.id) && u)
				user = await this.client.users.fetch(query, { cache: false });
		} catch {
			return null;
		}
		return user;
	...
	webhook(
		...
		if (makeClient) return new WebhookClient({ id: info.id, token: info.token });
		return info;
	}

	async updateCovid19Info(): Promise<void> {
		const all = (await this.client.web.get("https://disease.sh/v3/covid-19/all"))
			.data;
		const countries = (
	...
	async updateSlashCommands(): Promise<void> {
		let commands = await (this.client.Util.checkTesting()
			? this.client.guilds.cache
					.get(this.client.config.test_guild)
					.commands.fetch()
