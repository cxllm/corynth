    ...
    switch (arg) {
      case "on":
        ...
        let chan =
          ...
          msg.guild.channels.cache.get(channels.first().content);
      ...
      case "off":
        ...
        await this.client.db.guilds.set(msg.guild.id, msg.db);
        await msg.send(
          `${this.client.config.emojis.tick} Join message turned off`
        );
        break;
      case "view":
        ...
        embed = {
          title: "Join Message",
          description: join
            ? join.message && join.channel
              ? `The join message is set to \`${join.message}\` & the channel is <#${join.channel}>`
              : `The join message is off`
            : `The join message is off`,
          color: this.client.config.colours.main,
        };
