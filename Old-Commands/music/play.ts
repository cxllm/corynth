...
export = class extends Command {
  ...
  async run(msg: Message) {
    if (!msg.member.voice.channel)
      return await msg.send(
        `${this.client.config.emojis.cross} You aren't connected to a voice channel!`
      );
    let queue = this.client.queue.get(msg.guild.id);
    if (queue) {
      if (msg.member.voice.channel.id != queue.voice.id)
        return await msg.send(
          `${this.client.config.emojis.cross} Music is already playing in another channel!`
        );
    }
    ...
    if (song.live)
      return await msg.send(
        `${this.client.config.emojis.cross} Livestreams can't be played due to issues with playing them.`
      );
  ...
  async handle(
    ...
    song: Track,
    loadmsg: Message,
    playlist: { playlistName?: string; tracks: Track[] }
  ) {
    ...
    if (before > 0) {
      ...
      try {
        ...
        return await msg.send(
          `${client.config.emojis.cross} An error occured and I couldn't join the voice channel.`
        );
      }
  }
  async play(
    ...
    if (!song.track) {
      ...
      if (info?.tracks) song.track = info.tracks[0].track;
        ...
        let info2 = await client.music.rest.resolve(
