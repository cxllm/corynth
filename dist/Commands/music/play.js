...
module.exports = class extends Command_1.default {
    ...
    async run(msg) {
        ...
        try {
            if (playlistid) {
                const playlist = await ytpl_1.default(playlistid);
                results = {
        ...
        if (!song)
            return await msg.send(`${this.client.config.emojis.cross} Not found`);
        if (song.live)
            return await msg.send(`${this.client.config.emojis.cross} Livestreams can't be played due to issues with playing them.`);
    ...
    async play(client, guild, queue, song) {
        ...
        if (!song.track) {
            ...
            if (info === null || info === void 0 ? void 0 : info.tracks)
                song.track = info.tracks[0].track;
            else {
                let info2 = (await client.music.rest.resolve(`${song.info.title} ${song.info.author}`, `youtube`));
                if (info2 === null || info2 === void 0 ? void 0 : info2.tracks)
                    song.track = info2.tracks[0].track;
            }
        }
        if (!song.track)
