...
import ms from "ms";
import { GuildMember, Collection, User, WebhookClient, Activity, MessageAttachment } from "discord.js-light";
...
export default class Util {
    ...
    getCovidInfo(type: "country" | "world", country?: string) {
        ...
        if (type == "world") {
            ...
            return info.countries.find((c: { country: string, countryInfo: { iso2?: string } }) => country == c.country.toLowerCase() || c.countryInfo.iso2?.toLowerCase() == country);
        }
    }
    /**
     * 
    ...
    async getMember(ctx: Message, query?: string, cache?: boolean) {
        ...
        let out = members.get(id) ||
            ...
            || ctx.member;
        if ((out === ctx.member && [ctx.author.id, ctx.author.tag,].includes(query)) || out != ctx.member) return out;
    ...
    */
    ...
    getSpotifyInfo(member: GuildMember) {
        const info: Activity = member.presence.activities.find(act => act.name === "Spotify" && act.type === "LISTENING");
    ...
    memory(bytes: number, type?: string): number {
        if (type === "gb") return Math.round(bytes / 1024 / 1024 / 1000 * 10) / 10
     ...
     */
    webhook(string: string, makeClient: boolean): { id: string, token: string } | WebhookClient {
    ...
    async updateCovid19Info(): Promise<void> {
        ...
        writeFileSync(`${__dirname}/../covid-19.json`, `${JSON.stringify(data)}`);
    }
}
