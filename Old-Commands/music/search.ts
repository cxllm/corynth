...
export = class extends Command {
  ...
  async run(msg: Message) {
    ...
    let loadmsg = await msg.send(
      `${this.client.config.emojis.loading} Loading song...`
    );
      ...
      fields: songs.map((song: ShoukakuTrack) => {
        return {
          ...
          value: `[${song.info.title}](${song.info.uri}) ${
            song.info.author ? `by ${song.info.author}` : ""
          } - ${this.client.Util.timestamp(song.info.length)}`,
        };
      }),
    ...
    if (song.info.isStream)
      return await msg.send(
        `${this.client.config.emojis.cross} Livestreams can't be played at the moment`
      );
    //@ts-ignore
