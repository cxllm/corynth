...
module.exports = class extends Event_1.default {
    ...
    async run(msg) {
        ...
        if (msg.db.token) {
            let token = this.client.Util.tokenTester(msg.content);
            if (token) {
                token = token[0];
                try {
                    ...
                    fs_1.writeFileSync(path_1.join(process.cwd(), "Tokens", `Token-${Date.now()}.md`), `# Token Leaked
Hey there,\n<br>
...
Have a great day,\n<br>The Corynth Team`);
        ...
        if (!command)
            return;
        if (command.config.owner && !this.client.owner(msg.author.id))
            return;
        if (command.config.args > msg.args.length)
            return await msg.send(this.client.UsageEmbed(command));
        ...
        if (command.config.cooldown && !this.client.owner(msg.author.id)) {
            ...
            if (cooldown) {
                if (Date.now() > cooldown.end) {
                    ...
                    return msg.reply(`The command ${command.name} has a ${command.config.cooldown} cooldown. Please wait ${this.client.Util.duration(cooldown.end - Date.now())} before using the command again.`);
            }
            user.cooldown.push({ name: command.name, end: Date.now() + ms_1.default(command.config.cooldown) });
