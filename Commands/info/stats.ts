    ...
    const embed = {
      ...
      fields: [
        ...
        },
        {
          name: "Latency",
          value: [
            `Message Response: ${latency.msg.create}ms`,
        ...
        {
          name: "Music Stats",
          value: this.client.music
            ? [
                `Active Players: ${stats.playingPlayers}`,
                `Memory Usage: ${this.client.Util.memory(
                  stats.memory.used
                )} MB`,
                `CPU Usage: ${Math.round(stats.cpu.lavalinkLoad * 100) / 100}%`,
                `Uptime: ${this.client.Util.timestamp(stats.uptime)}`,
              ]
